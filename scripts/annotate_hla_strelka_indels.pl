...
@t = `ls $dir/*all.somatic.indels.vcf`;
$csFile = $t[0];
chomp($csFile);
open CSFILE, $csFile || die "Can not open $csFile\n";
while(($header = <CSFILE>) !~ /^#CHROM/){}
chomp($header);
$header = substr($header,1);
close CSFILE;

#print OUTFILE "individual\t$header\tstatus\texon_number\tintron_number\tprotein_change\tremainder\tframe_shift_status\n";
print OUTFILE "individual\t$header\texon_number\tintron_number\tprotein_change\n";

# get the list of strelka indel calls

@t=`grep ^hla $dir/*all.somatic.indels.vcf`;
...
for($i1 = 0; $i1 <= $#t; $i1++){
	...
	@f = split(/\t/,$line);
        ...
        $refLen = length($ref);
        $altLen = length($alt);
        print "$indiv\t$id\t$ref\t$alt\t$refLen\t$altLen\n";
        if($altLen > $refLen){
                $change = "insertion";
                $changeLen = $altLen - $refLen;
        }elsif($refLen > $altLen){
                $change = "deletion";
                $changeLen = $refLen - $altLen;
        } else{
                die "$id\t$line: this is not an indel output\n";
        }
        ...
        #print "$indiv\t$id\tstatus=$status\tpos=$pos\tchangeLen=$changeLen\texCount=$exonCount\t$intronCount\n";
	#check if it's a splice site mutation
        ...
        if($status eq "exon"){
                ...
                if($rem == 1){
                        ...
                        $cdna = $cdna.substr($f2[$i],$diff);
                        if($change eq "insertion"){
                                $cdna2 = $cdna2.substr($f2[$i],$diff);
                        } else {
                                $cdna2 = $cdna2.substr($f2[$i],$diff+$changeLen);
                        }
                        if($changeLen % 3 == 0){
                                $fsStatus = "no";
                        } else {
                ...
                if($isSplice == 1){
                        print "there in an intronic splice site indel\n";
                }

        }
        print "$ref\t$alt\tproteinChange = $proteinChange\n";
        #print OUTFILE "$indiv\t$line\t$status\t$exonCount\t$intronCount\t$proteinChange\t$rem\t$fsStatus\n";
	print OUTFILE "$indiv\t$line\t$exonCount\t$intronCount\t$proteinChange\n";
        #die;
}
